...
Quoin, following the same pattern as
MermaidKit: a small, published,
zero-dependency Swift package that a host app consumes from GitHub.

The current release is **1.5.0**. Roughly **~400 symbol commands** and over
...
- The package is **Swift 6, strict concurrency** (`swift-tools-version: 6.2`).
...
declares it as `.package(url: "…/Vinculum.git", from: "1.5.0")`. It is not
